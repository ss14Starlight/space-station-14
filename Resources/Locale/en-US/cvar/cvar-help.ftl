...
changecvar-simple-events_repetition_falloff = How much less likely an event is to repeat.
changecvar-full-events_repetition_falloff = Multiplier applied to an event's weight for each time it already ran this round. At 0.6 a repeat keeps 60% of its weight, then 36%, then 21%, so repeats become progressively less likely without ever being ruled out. Set to 1 to disable the penalty. Clamped to 0-1.

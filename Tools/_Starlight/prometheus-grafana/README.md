...
| `job_selection` | job | `candidates`, `available_slots`, `slots_recorded`, `slots_unlimited`, `round_start_assignments`, `late_join_assignments` |
| `job_spawn` | species, job and phase | `species_id`, `job_id`, `spawn_phase`, `count` |
| `antag_spawn` | antagonist type | `type_id`, `count` |
| `antag_selection` | rule and type | `expected`, `eligible`, `preselected`, `assigned`, `ghost_roles`, `unassigned`, `uncovered`, `forced_assignments`, `ghost_roles_created`, `latejoin_assignments` |
...
| `store_currency` | store and currency | `spent`, `remaining`, `stores` |

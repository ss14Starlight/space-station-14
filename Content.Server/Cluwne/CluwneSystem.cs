...
public sealed class CluwneSystem : EntitySystem
{
    ...
    private void OnComponentStartup(Entity<CluwneComponent> ent, ref ComponentStartup args)
    {
        ...
        #region Starlight
        if (ent.Comp.IsCluwne)
        {
            _popupSystem.PopupEntity(Loc.GetString("cluwne-transform", ("target", ent.Owner)), ent.Owner, PopupType.LargeCaution);
            _audio.PlayPvs(ent.Comp.SpawnSound, ent.Owner);

            _outfitSystem.SetOutfit(ent.Owner, "CluwneGear");
            _faction.RemoveFaction(ent.Owner, "NanoTrasen", false);
            _faction.AddFaction(ent.Owner, "HonkNeutral");
        }
        else
        {
            Spawn(ent.Comp.Portal, Transform(ent.Owner).Coordinates);
            _outfitSystem.SetOutfit(ent.Owner, "CluwneBeastGear");
            _audio.PlayPvs(ent.Comp.ArrivalSound, uid);
        }
        #endregion Starlight

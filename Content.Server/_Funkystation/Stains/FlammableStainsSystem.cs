...
public sealed partial class FlammableStainsSystem : EntitySystem
{
    ...
    private float _burnAccumulator; // Starlight

    private const float BurnInterval = 1f; // Starlight
    ...
    private SlotFlags GetBlockedSlots(EntityUid wearer, InventoryComponent inv) // Starlight
    {
        var blockedSlots = SlotFlags.NONE; // Starlight
        foreach (var slot in inv.Slots)
        {
            ...
            if (TryComp<StainBlockerComponent>(slotEnt, out var blocker))
                blockedSlots |= blocker.BlockedSlots; // Starlight
        }

        return blockedSlots; // Starlight
    }

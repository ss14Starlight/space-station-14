...
using Content.Server._Funkystation.BloodCult;
using Content.Server._Funkystation.BloodCult.Components;
using Content.Shared._Funkystation.BloodCult;
...
using Content.Server._Funkystation.GameTicking.Rules;
...
namespace Content.Server._Funkystation.BloodCult.EntitySystems
{
	...
	private bool _HasBrainPrototype(EntityUid uid)
	{
		...
		{
			if (_body.GetBodyOrgans(uid, body).Any(o => HasComp<BrainComponent>(o.Id)))
				return true;
		}
	...
	private bool CreateSoulstoneInternal(EntityUid victim, EntityCoordinates coordinates, EntityUid? user, bool showPopup, bool autoActivateShade = false)
	{
		...
		{
			// Simple iteration to find and extract brain
			foreach (var (organId, organComp) in _body.GetBodyOrgans(victim, body))
			{
				if (!HasComp<BrainComponent>(organId))
					continue;
					
				var brainMeta = MetaData(organId);
				if (brainMeta.EntityPrototype != null && !isHamlet)
					originalEntityPrototype = brainMeta.EntityPrototype.ID;
			...
			// If no brain was found, use victim's prototype directly (handles skeletons, etc.)
			else if (victimMeta.EntityPrototype != null && !isHamlet)
			{
		...
		{
			// No body component - use victim's prototype directly

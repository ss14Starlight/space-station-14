...
public sealed class MonumentSystem : SharedMonumentSystem
{
    ...
    /// </summary>
    private void OnShuttleEvac(ref EvacShuttleLeftEvent args)
    {
    ...
    public void UpdateMonumentReqsForTier(Entity<MonumentComponent> monument, int tier)
    {
        ...
        var numberOfCrewForTier3 = Math.Round((double)cult.Comp.TotalCrew / 100 * _config.GetCVar(StarlightCCVars.CosmicCultTargetConversionPercent)); // 40% of current pop

        switch (tier)
        {
            case 1:
                monument.Comp.ProgressOffset = 0;
                monument.Comp.TargetProgress = (int)(numberOfCrewForTier3 / 2 * _config.GetCVar(StarlightCCVars.CosmicCultistEntropyValue));
                break;
            case 2:
                monument.Comp.ProgressOffset = (int)(numberOfCrewForTier3 / 2 * _config.GetCVar(StarlightCCVars.CosmicCultistEntropyValue)); //reset the progress offset
                monument.Comp.TargetProgress = (int)(numberOfCrewForTier3 * _config.GetCVar(StarlightCCVars.CosmicCultistEntropyValue));
                break;
            case 3:
                monument.Comp.ProgressOffset = (int)(numberOfCrewForTier3 * _config.GetCVar(StarlightCCVars.CosmicCultistEntropyValue));
                monument.Comp.TargetProgress = (int)(numberOfCrewForTier3 * _config.GetCVar(StarlightCCVars.CosmicCultistEntropyValue)); //removed offset; replaced with timer
                break;
        }
    }

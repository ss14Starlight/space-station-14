...
/// and recovery. A host carries at most one strain. Transmission between hosts lives in
/// <see cref="PathogenSpreadSystem"/>.
/// </summary>
public sealed partial class PathogenSystem : EntitySystem
{
    ...
    private void ExpressSymptoms(EntityUid uid, PathogenInfection infection, Pathogen strain, TimeSpan curTime)
    {
        ...
        foreach (var symptomId in strain.Symptoms)
        {
            ...
            var scale = 1f + (infection.Stage - 1) * symptom.StageScaling;
            foreach (var effect in symptom.Effects)
                _effects.TryApplyEffect(uid, effect, scale);
        }
    }
    ...
    /// </summary>
    public bool TryInfect(EntityUid uid, int strainId, bool bypassImmunity = false)
    {
    ...
    /// </summary>
    public bool Cure(EntityUid uid, int strainId, bool grantImmunity = false)
    {

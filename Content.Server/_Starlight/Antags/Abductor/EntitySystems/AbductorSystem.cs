...
public sealed partial class AbductorSystem : SharedAbductorSystem
{
    ...
    private void OnAbductorBeaconChosenBuiMsg(Entity<AbductorHumanObservationConsoleComponent> ent, ref AbductorBeaconChosenBuiMsg args)
    {
        ...
        if (ent.Comp.RemoteEntityProto != null)
        {
            ...
            {
                var actor = (args.Actor, handsComponent);
                foreach (var hand in _hands.EnumerateHands(actor))
                {
                    if (_hands.HandIsEmpty(actor, hand))
                        continue;
                    if (HasComp<UnremoveableComponent>(_hands.GetHeldItem(actor, hand)))
                        continue;

                    _hands.DoDrop(actor, hand, true);
                }

...
public sealed partial class IPCSystem
{
    ...
    /// Deals 50 shock damage to the IPC and electrocutes the user.
    ...
    private void OnBeforeZap(Entity<IPCReviveComponent> ent, ref TargetBeforeDefibrillatorZapsEvent args)
    {
        ...
        // Deal 50 shock damage to the IPC (damages them instead of helping)
        var damage = new DamageSpecifier();
        damage.DamageDict.Add("Shock", 50);
        _damageable.TryChangeDamage(ent.Owner, damage);
        
        // Electrocute the user (125 damage, 5 second stun)
        _electrocution.TryDoElectrocution(args.EntityUsingDefib, null, 125, TimeSpan.FromSeconds(5), true);
    ...
    private void AddReviveVerbs(Entity<IPCReviveComponent> ent, ref GetVerbsEvent<Verb> ev)
    {
        if (!ev.CanInteract || !ev.CanAccess || !ev.CanComplexInteract ||
            // IPCLockComponent not implemented yet, skip lock check

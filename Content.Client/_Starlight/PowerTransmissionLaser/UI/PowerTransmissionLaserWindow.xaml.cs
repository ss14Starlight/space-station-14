...
public sealed partial class PowerTransmissionLaserWindow : DefaultWindow
{
    ...
    public PowerTransmissionLaserWindow()
    {
        ...
        _batteryRsi = resCache.GetResource<RSIResource>("/Textures/Interface/Alerts/battery.rsi").RSI;
    ...
    private void SetBatteryIconState(string state)
    {
        if (!_batteryRsi.TryGetState(new RSI.StateId(state), out var rsiState))
            return;

        BatteryIcon.Texture = rsiState.Frame0;
    }

    private static void SetToggleVisual(Button button, bool enabled) => button.Text = enabled ? Loc.GetString("ptl-ui-enabled") : Loc.GetString("ptl-ui-disabled");
    ...
    public void SetBattery(float currentJ, float maxJ, float reservedCurrentJ, float reservedMaxJ)
    {
        ...
        if (combinedMax <= 0f)
        {
            SetBatteryIconState("battery-none");
            return;
        }
        ...
        SetBatteryIconState($"battery{level}");
    }

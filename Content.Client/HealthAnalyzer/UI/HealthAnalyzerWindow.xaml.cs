...
namespace Content.Client.HealthAnalyzer.UI
{
    ...
    public sealed partial class HealthAnalyzerWindow : FancyWindow
    {
        ...
        public void Populate(HealthAnalyzerScannedUserMessage msg)
        {
            ...
            DrawStatusEffects(msg.StatusEffects);
            // Starlight end
        }
        ...
        private void DrawStatusEffects(List<(string StatusEffectId, TimeSpan TimeRemaining)>? statusEffects)
        {
            StatusEffects.RemoveAllChildren();

            var hasStatusEffects = statusEffects != null && statusEffects.Count > 0;

            StatusEffectsDivider.Visible = hasStatusEffects;
            StatusEffects.Visible = hasStatusEffects;

            if (!hasStatusEffects || statusEffects == null)
                return;

            // Sort by quantity descending
            var sortedEffects = statusEffects.OrderByDescending(r => r.TimeRemaining).ToList();

            foreach (var effect in sortedEffects)
            {
                var statusEffectName = effect.StatusEffectId;
                var time = effect.TimeRemaining;
                
                if (_prototypes.TryIndex<EntityPrototype>(effect.StatusEffectId, out var reagentProto))
                {
                    statusEffectName = reagentProto.Name;
                }
                ...
                rowContainer.AddChild(nameLabel);
                rowContainer.AddChild(timeLabel);
                StatusEffects.AddChild(rowContainer);
            }
        }
        // Starlight end

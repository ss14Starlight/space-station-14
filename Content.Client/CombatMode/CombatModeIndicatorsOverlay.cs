...
public sealed class CombatModeIndicatorsOverlay : Overlay
{
    ...
    private void UpdateCursor(Control? viewport, bool hidden)
    {
        ...
        if (_hiddenCursorViewport == target)
            return;

        _hiddenCursorViewport?.CustomCursorShape = null;

        _hiddenCursorViewport = target;
        if (target == null)
            return;

        _blankCursor ??= _clyde.CreateCursor(new SixLabors.ImageSharp.Image<Rgba32>(32, 32), Vector2i.Zero);
        target.CustomCursorShape = _blankCursor;
    }
    ...
    private float CalculateOffset(float spreadDegrees, float distance)
    {
        // Shots deviate up to half of the spread cone to either side.
        var halfAngleRad = spreadDegrees * 0.5f * MathF.PI / 180f;
        var offset = MathF.Tan(halfAngleRad) * distance;
    ...
    private void DrawSightPartial(Texture sight, Texture bracket1, Texture bracket2, DrawingHandleScreen screen, float rotation, Vector2 centerPos, float scale, Color mainColor, Color strokeColor, float offset, bool rotate, Texture? bracket3 = null, Texture? bracket4 = null)
    {

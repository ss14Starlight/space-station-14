...
public sealed partial class ShadekinSystem : EntitySystem
{
    ...
    {
        // TODO STARLIGHT this method is a catastrophe and should either be using a component tree system or at least multithreaded where it can be
        var illumination = 0f;
        ...
        foreach (var light in lightQuery)
        {
            ...
            if (lightComp.MaskPath is not null)
            {
                var angleToTarget = GetAngle(light, lightComp, uid);
                foreach (var cone in _lightMasks[lightComp.MaskPath])
                {
                    float coneLight;

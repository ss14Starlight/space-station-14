...
public sealed partial class RodentiaBagJumpSystem : EntitySystem
{
    ...
    private void OnDoAfter(Entity<StorageComponent> ent, ref RodentiaBagJumpDoAfterEvent args)
    {
        ...
        if (!args.Attached && (args.Target != ent.Owner || _container.IsEntityInContainer(ent.Owner)))
            return;
    ...
    {
        return _inventory.TryGetSlotEntity(wearer, "back", out var back) && back == bag && IsBag(bag);
    }

    private bool IsBag(EntityUid uid)
    {
        return TryComp<StorageComponent>(uid, out _)
    ...
    public sealed partial class RodentiaBagJumpDoAfterEvent : DoAfterEvent
    {
        ...
        private RodentiaBagJumpDoAfterEvent()
        {
        }

        public RodentiaBagJumpDoAfterEvent(bool attached)
        {
            Attached = attached;
        }
